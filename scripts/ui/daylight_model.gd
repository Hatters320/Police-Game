...
static func daylight_factor(minute_of_day: int, season: GameEnums.Season) -> float:
	var sunrise: int = SeasonCycle.sunrise_minute(season)
	var sunset: int = SeasonCycle.sunset_minute(season)
	var twilight: int = SeasonCycle.twilight_minutes(season)
	...
	if minute_of_day >= sunrise and minute_of_day < sunrise + twilight:
		return float(minute_of_day - sunrise) / float(twilight)
	if minute_of_day > sunset - twilight and minute_of_day <= sunset:
		return float(sunset - minute_of_day) / float(twilight)
	return 0.0

## True while the sun is up at all -- used for the warm-vs-cool decision
## and by callers that just want "is it dark out".
static func is_daylight(minute_of_day: int, season: GameEnums.Season) -> bool:
...
static func sun_rotation_degrees(minute_of_day: int, season: GameEnums.Season) -> Vector3:
	var sunrise: int = SeasonCycle.sunrise_minute(season)
	var sunset: int = SeasonCycle.sunset_minute(season)
	var progress: float = 0.5
	if sunset > sunrise:
		progress = clampf(float(minute_of_day - sunrise) / float(sunset - sunrise), 0.0, 1.0)
...
static func sun_color(minute_of_day: int, season: GameEnums.Season) -> Color:
	...
	var horizon_tint: Color = SUN_DAWN if float(minute_of_day) < midpoint else SUN_DUSK
...
static func sun_energy(minute_of_day: int, season: GameEnums.Season) -> float:
	return lerpf(SUN_ENERGY_NIGHT, SUN_ENERGY_DAY, daylight_factor(minute_of_day, season))

static func ambient_color(minute_of_day: int, season: GameEnums.Season) -> Color:
	return AMBIENT_NIGHT.lerp(AMBIENT_DAY, daylight_factor(minute_of_day, season))

static func ambient_energy(minute_of_day: int, season: GameEnums.Season) -> float:
	return lerpf(AMBIENT_ENERGY_NIGHT, AMBIENT_ENERGY_DAY, daylight_factor(minute_of_day, season))

static func sky_color(minute_of_day: int, season: GameEnums.Season, weather: GameEnums.WeatherType = GameEnums.WeatherType.CLEAR) -> Color:
	var factor: float = daylight_factor(minute_of_day, season)
	var midpoint: float = float(SeasonCycle.sunrise_minute(season) + SeasonCycle.sunset_minute(season)) * 0.5
	var horizon: Color = SKY_DAWN if float(minute_of_day) < midpoint else SKY_DUSK
